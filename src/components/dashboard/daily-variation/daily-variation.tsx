...
import { ChangePercentage, Spinner } from '@/components/ui/common';
...
export const DailyVariation = (): JSX.Element => {
  const { data, isLoading } = useGetCoinVariation(coinId);
  const { info, variation = [] } = data || {};

  return (
    <div className="shadow-dashboard tablet:flex-row tablet:max-h-[112px] flex h-full max-h-[142px] w-full flex-col rounded-lg bg-white">
